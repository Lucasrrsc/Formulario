document.addEventListener('DOMContentLoaded', function () {
    ...
    formulario.addEventListener('submit', function (event) {
        ...
        const atividadeCheckboxes = document.querySelectorAll('input[name="atividade"]:checked');
        ...
        if (nomeInput.value.trim().length < 3) {
            ...
            displayError(nomeInput, "O campo nome deve conter pelo menos 3 caracteres.");
        } else {
        ...
        if (datainiInput.value.trim() === '') {
            ...
            displayError(datafimInput, "A Data Final deve ser posterior à Data Inicial.");
        } else {
        ...
        if (datafimInput.value.trim() === '') {
            ...
            datainiInput.style.border = "1px solid red";
            displayError(datainiInput, "A Data Inicial não pode ser anterior à data atual.");
        }
        ...
        if (websiteURL.protocol === "http:" || websiteURL.protocol === "https:") {
            ...
            errors.push("O campo Website deve estar em um formato válido (http[s]://xxx.xx).");
            websiteInput.style.border = "1px solid red";
            displayError(websiteInput, "O campo Website deve estar em um formato válido (http[s]://xxx.xx).");
        }
        } catch (error) {
            errors.push("O campo Website deve estar em um formato válido (http[s]://xxx.xx).");
            websiteInput.style.border = "1px solid red";
        ...
        if (atividadeCheckboxes.length < minAtividades || atividadeCheckboxes.length > maxAtividades) {
            errors.push(`Selecione de ${minAtividades} a ${maxAtividades} atividades pretendidas.`);
        ...
        if (!regiaoSelecionada) {
            ...
            displayError(reg, "Selecione a região.");
            regiaoRadios.forEach(function (radio) {
        });
        }
        ...
        const generalErrorDiv = document.getElementById('error-message');
            ...
            generalErrorDiv.innerHTML = "Por favor, corrija os erros no formulário.<br> Preencha todos os campos obrigatórios. (*)";
